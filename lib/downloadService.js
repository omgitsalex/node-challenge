...
module.exports = {
	downloadAndExtractPackages
};

function downloadAndExtractPackages(packageArray, callback) {
	...
	async.each(packageArray, (element, callback) => {
		...
		exec(`npm view ${element.name} dist.tarball`, (err, stdout, stderr) => {
			if (!err) {
				...
				tarball.extractTarballDownload(url,`${tarballDirectory}/${element.name}.tgz`, elementDirectory, {}, (err, result) => {
					if (!err) {
